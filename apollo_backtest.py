...
class Stock():
    ...
    def prepare_model_data(self):
        ...
        self.df['Long_GNG'] = 0.0
        for i in range(self.df.shape[0]):
            # Search the next look_ahead_periods to find the highest closing value
            LA_Max = self.df['High'][i:i + self.look_ahead_periods].max()
        
            # Find close of current row for loop calcs
            row_close = self.df['Close'][i]
        
            # Fraction change of the max closing value in the next look_ahead_periods
            fraction = (LA_Max - row_close)/row_close
            ...
            self.df.loc[index, 'Long_GNG'] = Long_GNG
...
def find_exit_data(eod_df, index, target):
    req_exit_date = eod_df.iloc[index + 30]['Date']
    for i in range(index,index+31):
        daily_high = eod_df.iloc[i]['High']
        ...
        if daily_high >= target:
            ...
            exit_price = eod_df.iloc[index + 30]['Open']
            hit_target = False
...
send_text_message(phone_number_list, 'Apollo backtest started.')
...
backtest_filename = 'apollo-backtest_' + today_date + '.csv'
...
today_date = str(datetime.date.today())
apollo_accuracy_filename = 'apollo-accuracy_' + today_date + '.csv'
f = open(apollo_accuracy_filename, 'w')
...
for i in range(len(backtest_data)):
    try:
        if backtest_data.iloc[i]['prediction'] >= prediction_threshold:
            ...
            target = eod_data.iloc[entry_index - 1]['Close'] * 1.2
            entry_price = eod_data.iloc[entry_index]['Open']
            highest_high = eod_data['High'][entry_index:(entry_index+31)].max()
            lowest_low = eod_data['Low'][entry_index:(entry_index+31)].min()
            exit_date, exit_price, hit_target, req_exit_date = find_exit_data(eod_data, entry_index, target)
            f = open(apollo_accuracy_filename, 'a')
            f.write('{},{},{},{},{},{},{},{},{},{},{},{},{}{}'.format(ticker_symbol,target_fraction,prediction,target,prediction_date,entry_date,exit_date,req_exit_date,entry_price,exit_price,highest_high,lowest_low,hit_target,'\n'))
            f.close()
            print(ticker_symbol,prediction_date,hit_target)
            counter += 1
    except:
        except_counter +=1
        pass
...
send_text_message(phone_number_list, 'Apollo backtest finished.')
